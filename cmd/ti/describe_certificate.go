...
func newDescribeCertificateCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runDescribeCertificate(config, args[0], koap.CertRef(args[1]))
		},
	}
}

func runDescribeCertificate(config *koap.Dotkon, cardHandle string, certRef koap.CertRef) error {
	client, err := loadClient(config)
	if err != nil {
		return err
	}

	certs, err := client.ReadCardCertificates(cardHandle, certRef)
	if err != nil {
		return err
	}
