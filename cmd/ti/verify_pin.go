...
func newVerifyPinCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:      fmt.Sprintf("Verify PIN of a card.\nValid PIN types: %s", koap.PinTypValuesString()),
		Args:      cobra.ExactArgs(2),
		ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			if len(args) == 1 {
				return koap.PinTypValues(), cobra.ShellCompDirectiveNoFileComp
			}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			pinTyp := koap.PinTyp(args[1])
			if !pinTyp.IsValid() {
				return fmt.Errorf("invalid PIN type %q, valid types: %s", args[1], koap.PinTypValuesString())
			}
...
func runVerifyPin(ctx context.Context, config *koap.Dotkon, cardHandle string, pinTyp koap.PinTyp) error {
